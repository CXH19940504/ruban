...
class DkClient(BaseModel):
    ...
    s_name = mapped_column(String(10), nullable=False, comment='节点名称')
    client_id = mapped_column(String(50), nullable=False, comment='得捷配置')
...
if __name__ == '__main__':
    ...
    with Session(get_engine(db)) as session:
        ins_stat = insert(DkClient).values(
            name='test',s_name='s',client_id="ervghfehorfhuiq12",client_secret='afvwefaef',status=1)
